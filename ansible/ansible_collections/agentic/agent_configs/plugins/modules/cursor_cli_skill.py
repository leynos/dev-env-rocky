"""Manage Cursor CLI skill directories.

Cursor skills are file-backed instructions that the Cursor agent can discover
from user-scoped ``~/.cursor/skills`` or project-scoped ``.cursor/skills``
directories.
"""
...
DOCUMENTATION = r"""
---
module: cursor_cli_skill
short_description: Manage Cursor CLI skills
version_added: "1.0.0"
description:
  - Manage Cursor skill directories containing C(SKILL.md) and optional support files.
options:
  name:
    description:
      - Skill display name.
...
def build_frontmatter(module: AnsibleModule) -> dict:
    """Build Cursor skill front matter."""
...
def resolve_directory(module: AnsibleModule) -> str:
    """Resolve the managed Cursor skill directory."""
    ...
    try:
        return resolve_scoped_config_path(
            ...
            user_path=os.path.join("~/.cursor/skills", slug),
            project_relative_path=os.path.join(".cursor", "skills", slug),
        )
