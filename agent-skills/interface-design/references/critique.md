...
There's a distance between correct and crafted. Correct means the layout holds, the grid aligns, the colours don't clash. Crafted means someone cared about every decision down to the last pixel. The difference is felt immediately — the way a viewer distinguishes a hand-thrown mug from an injection-moulded one. Both hold coffee. One has presence.
...
Typography should be legible even squinted. If size is the only thing separating the headline from the body from the label, the hierarchy is too weak. Weight, tracking, and opacity create layers that size alone can't.

Surfaces should whisper hierarchy. Not thick borders, not dramatic shadows — quiet tonal shifts where the viewer feels the depth without seeing it. Mentally remove every border from the Cascading Style Sheets (CSS). Can the structure still be perceived through surface colour alone? If not, the surfaces aren't working hard enough.
